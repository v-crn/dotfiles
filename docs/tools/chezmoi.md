...
runtime hook は `home/dot_agents/hooks/` や `home/dot_claude/hooks/` のような通常ファイルとして管理し、`chezmoi apply` でホームディレクトリへデプロイする。

`.chezmoiscripts/` や `run_` スクリプトは設定ファイルの生成・マージのために使い、ランタイムでエージェントから直接呼ばれる hook 本体の置き場にはしない。
